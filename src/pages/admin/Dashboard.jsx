...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-[#080c10] grid place-items-center text-[var(--accent)]">Loading Dashboard...</div>
  if (error) return (
    <div className="min-h-screen bg-[#080c10] grid place-items-center text-center">
      <div>
        <h2 className="text-2xl text-red-400 mb-4">{error}</h2>
        <a href="/admin-login" className="px-6 py-3 bg-[var(--accent)] text-black rounded font-bold">Go to Login</a>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#080c10]">
      <nav className="bg-[#0c1218] border-b border-white/10 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded bg-[var(--accent)] flex items-center justify-center text-black font-bold">A</div>
          <span className="text-white font-bold text-lg">Admin Dashboard</span>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-gray-400 text-sm hidden md:inline">Logged in as Admin</span>
          <button onClick={() => window.location.href = '/'} className="text-gray-400 hover:text-white transition">Exit</button>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage RFQ submissions and status.</p>
          </div>
          <div className="text-[var(--accent)] font-bold text-xl">{rfqs.length} <span className="text-gray-500 text-base font-normal">Total</span></div>
        </div>
